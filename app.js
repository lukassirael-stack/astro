(function () {
  ...
  function guideHTML() {
    const SEC = [
      ...
      ['ridit', '✦', 'Čím se řídit', `<b>Barva dne</b> je souhrn — jediné číslo, které bere všechno v úvahu. <b>Dvě věty v Dnes</b> — <em>u tebe</em> (co se dotýká tvé mapy) a <em>Krok dne</em> (jeden krok) — víc pokynů den nemá. <b>Všechno ostatní jsou vrstvy</b>: tatva, tělo, zahrádkář, zlatá hodina, čísla, kolo roku. Říkají, co je, ne co dělat. Kdo si je oblíbí, čte je; kdo ne, nic nezmešká.`],
      ...
      ['dnes', '◉', 'Karta Dnes', `Řádky odshora: <b>hodina</b> (planetární hodina a na co se hodí), <b>cena dne</b> (co dnes bude stát víc sil), <b>tatva</b> (jemný rytmus po 24 minutách od východu Slunce), <b>tělo</b> (orgánové hodiny ukotvené na skutečné poledne), <b>u tebe</b> (nejsilnější tranzit na tvou mapu a kde v jeho oblouku stojíš). Otazník u každého řádku vysvětlí, co je zač. Dole je <b>Krok dne</b>: otázka a jeden krok.`],
  ...
  function wxLine() {
    ...
    return `<span class="ht-wx ${S.wxOpen ? 'open' : ''}" role="button" tabindex="0" data-act="wxToggle" title="venku teď · ${WX_CZ(w.c)}" aria-expanded="${!!S.wxOpen}"><i>${WX_EMO(w.c)}</i><b>${w.t}<span class="dg">°</span></b><small>${w.tmin}° / ${w.tmax}°</small></span>`;
  }
