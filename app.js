(function () {
  ...
  function guideHTML() {
    ...
    const VLIVY = [
      ...
      ['zatmeni', 'Zatmění', `Nov nebo úplněk těsně u lunárních uzlů. Tradičně silné body roku: zatmění Slunce otevírá půlroční kapitolu, zatmění Luny něco uzavírá. Kompas říká, ve kterém znamení a domě tvé mapy zatmění padne, a jestli je u vás vidět.`],
      ['kp', 'Kosmické počasí', `Index Kp (0–9) měří geomagnetickou aktivitu — bouře po slunečních erupcích. Nad 5 je bouře: víc polárních září, ale i únava, nespavost a neklid u citlivých lidí. Kompas stahuje aktuální Kp a započítává ho do barvy dne a do elekce; jde vypnout v Nastavení.`],
      ['hvezdy', 'Stálice', `Dvacet dva hvězd a světů — Plejády, Sirius, Arkturus, Vega, Andromeda… — s polohou přepočtenou na dnešek. Hvězda, která při tvém narození stála na některém tvém bodě nebo na úhlu, je tvá hlavní hvězda: má rod, dar, stín a úkol. Dny, kdy přes ni přechází planeta nebo Luna, jsou rezonanční (✦ v kalendáři). Heliakický východ je den, kdy se hvězda po období ve slunečním žáru poprvé zase objeví ráno.`],
      ['cykly', 'Velké cykly života', `Sluneční návrat je tvůj osobní nový rok. Saturn se vrací kolem 29 a 59 let (zkouška dospělosti, pak moudrosti), Jupiter každých 12 (nový cyklus růstu), Uran stojí naproti kolem 41 (polovina života), uzly se vracejí každých 18,6 roku, Chiron kolem 50. Kompas je má ve Velkých návratech a v Úkazech pod <em>tvé cykly</em>.`],
      ['cisla', 'Numerologie', `Životní číslo je součet číslic data narození, zredukovaný na 1–9 (11, 22 a 33 zůstávají jako mistrovská). Osobní rok, měsíc a den se posouvají po devíti — rok 1 začíná, rok 9 uzavírá. K tomu číslo narozeniny, vrcholy a výzvy, karmické dluhy, mřížka a soulad životních čísel dvou lidí. Kompas to čte jako druhý pohled vedle horoskopu; kde se oba shodnou, tam je to jisté.`],
    ...
    const part = S.guidePart || 'kompas'; const list = part === 'vlivy' ? VLIVY : SEC;
    const tabs = `<div class="row nvtabs"><button type="button" class="chip ${part === 'kompas' ? 'on' : ''}" data-act="guidePart" data-p="kompas">Jak s Kompasem pracovat</button><button type="button" class="chip ${part === 'vlivy' ? 'on' : ''}" data-act="guidePart" data-p="vlivy">Průvodce vesmírnými vlivy</button></div>`;
    const ico = (x) => x.length > 2 ? x[1] : '✦';
  ...
  function natureMonthHTML(y, m) {
    ...
    for (let d = 1; d <= n; d++) {
      let ms, ph; try { const t = A.MakeTime(K.dayStart(y, m, d, TZ)); ms = K.signOf(K.lonOf('Moon', t)); ph = A.MoonPhase(t); } catch (e) { continue; }
      const el = ms % 4; kinds[el].push(d); (ph < 180 ? grow : wane).push(d);
    }
  ...
  function dayReadingHTML(y, m, d) {
    ...
    const rd = TX.dayReading(da, dayEv); const tip = taskOfDay(da);
    const arcs = transitArcs(y, m, d).filter(it => !it.slow).slice(0, 4);
    const gen = TX.generalItems(da, dayEv, rules()), per = TX.personalItems(da, rules());
    const go = gen.go.concat(per.go), cost = gen.cost.concat(per.cost); const phT = TX.phaseText(da.phaseAngle);
  ...
  function weekReadingHTML(y, m, d) {
    const start = K.dayStart(y, m, d, TZ); const p0 = K.tzParts(start, TZ); const off = (p0.wd + 6) % 7; const mon = new Date(start.getTime() - off * 86400000);
    const days = []; for (let i = 0; i < 7; i++) { const dt = new Date(mon.getTime() + i * 86400000); const p = K.tzParts(dt, TZ); let da; try { da = analyze(p.y, p.m, p.d); } catch (e) { continue; } days.push({ p, da, dt, tip: taskOfDay(da) }); }
    if (!days.length) return '';
    const best = days.slice().sort((a, b) => b.da.score - a.da.score)[0], hard = days.slice().sort((a, b) => a.da.score - b.da.score)[0];
    const q = K.moonQuartersBetween(mon, new Date(mon.getTime() + 7 * 86400000)).map(x => `${['novoluní', 'první čtvrt', 'úplněk', 'poslední čtvrt'][x.quarter]} ${K.tzParts(x.date, TZ).d}. ${K.tzParts(x.date, TZ).m}.`);
    const mr = monthReading(days[0].p.y, days[0].p.m); const moves = mr ? mr.moves.filter(x => days.some(dd => dd.p.m === days[0].p.m && dd.p.d === x.d)) : [];
    const avg = days.reduce((s, x) => s + x.da.score, 0) / days.length;
    const tone = avg >= 1.5 ? 'příznivý' : avg >= .5 ? 'vlídný' : avg > -.5 ? 'klidný' : avg > -1.5 ? 'pomalejší' : 'náročný';
    const first = days[0].p, lastp = days[6].p;
    return `<div class="card mrhead"><p class="lede">Týden ${first.d}. ${first.m}. – ${lastp.d}. ${lastp.m}. je celkově <b>${tone}</b>. Nejlepší den je <b>${K.WEEKDAY_CZ[best.p.wd]} ${best.p.d}. ${best.p.m}.</b>${hard.da.score < 0 ? `, nejtěžší <b>${K.WEEKDAY_CZ[hard.p.wd]} ${hard.p.d}. ${hard.p.m}.</b>` : ''}${q.length ? ` Luna: ${q.join(', ')}.` : ''}${moves.length ? ` ${moves.map(x => `${K.BODY_CZ[x.b]} vstupuje ${x.d}. ${x.h ? `do tvého ${x.h}. domu` : ''}`).join(', ')}.` : ''}</p></div>
      <div class="h3">Den po dni</div>
      ${days.map(x => `<div class="wkday ${x.da.color}" data-act="jumpDay" data-y="${x.p.y}" data-m="${x.p.m}" data-d="${x.p.d}" role="button"><div class="wkh"><b>${K.WEEKDAY_CZ[x.p.wd]} ${x.p.d}. ${x.p.m}.</b><span class="wkc">${dayWord(x.da)} den</span></div><p>${esc(x.tip.t)}</p><small>Luna ${K.SIGN_LOC_V[x.da.moonSign]} · ${esc(TX.phaseText(x.da.phaseAngle).name.replace(' Luna', ''))}</small></div>`).join('')}
      <p class="note" style="margin-top:10px">Týden začíná pondělím. Klepnutím na den ho otevřeš v kalendáři.</p>`;
  }
  function yearReadingHTML(y) {
    const n = S.natal; if (!n) return '';
    const prof = activeProfile(); const ny = numerology(prof, y, 1, 1);
    ...
    // lunace po měsících
    const lun = []; for (let mm = 1; mm <= 12; mm++) { const r = monthReading(y, mm); if (!r) continue; for (const l of r.lun) lun.push({ mm, ...l }); }
    const yearChapter = (HS_THEMES.find(t => t[0] === 'rok') || [])[2];
    return `<div class="card mrhead"><p class="lede">Rok ${y}: tvůj osobní rok <b>${ny.year}</b> — ${NUM_YEAR[ny.year]}.${sr ? ` Sluneční návrat, začátek tvého osobního roku, nastane ${K.tzParts(sr.date, TZ).d}. ${K.tzParts(sr.date, TZ).m}. v ${K.fmtTime(sr.date, TZ)}.` : ''}</p></div>
  ...
  // uvnitř tlačítka smí být jen řádkové prvky — bloky převést na span
  ...
  // denní (8–20 h) a noční (20 h – 7 h dalšího dne) teplota z hodinových dat; bez nich min/max
  function wxDayNight(w, key) {
    const hrs = w.hours || []; const next = (() => { const [y, m, d] = key.split('-').map(Number); const p = K.tzParts(new Date(Date.UTC(y, m - 1, d, 12) + 86400000), TZ); return K.isoDate(p.y, p.m, p.d); })();
    const day = hrs.filter(h => h.t.startsWith(key) && +h.t.slice(11, 13) >= 8 && +h.t.slice(11, 13) <= 20).map(h => h.temp);
    const night = hrs.filter(h => (h.t.startsWith(key) && +h.t.slice(11, 13) >= 20) || (h.t.startsWith(next) && +h.t.slice(11, 13) <= 7)).map(h => h.temp);
    return { day: day.length ? Math.round(Math.max(...day)) : null, night: night.length ? Math.round(Math.min(...night)) : null };
  }
  const wxTempTxt = (w, key, dd) => { const dn = wxDayNight(w, key); return dn.day != null && dn.night != null ? `ve dne ${dn.day}°, v noci ${dn.night}°` : `${dd.tmin}° až ${dd.tmax}°`; };
  ...
  function wxDetailHTML(y, m, d, tw, dn) {
    ...
    return `<div class="wxd">
      ${row(WX_EMO(day.c), 'počasí', `<b>${WX_CZ(day.c)}</b> · ${wxTempTxt(w, key, day)}${isToday ? ` · teď ${w.t}° (pocitově ${w.feels}°)` : ''} · vítr do ${day.wind} km/h`)}
  ...
  const actions = {
    ...
    guidePart(el) { S.guidePart = el.dataset.p; renderSettings(); window.scrollTo({ top: 0 }); },
    ...
    hdPrev() { const t = new Date(K.dayStart(S.hdY || np.y, S.hdM || np.m, S.hdD || np.d, TZ).getTime() - 86400000); const p = K.tzParts(t, TZ); S.hdY = p.y; S.hdM = p.m; S.hdD = p.d; renderNatal(); },
    hdNext() { const t = new Date(K.dayStart(S.hdY || np.y, S.hdM || np.m, S.hdD || np.d, TZ).getTime() + 86400000); const p = K.tzParts(t, TZ); S.hdY = p.y; S.hdM = p.m; S.hdD = p.d; renderNatal(); },
    hwPrev() { const t = new Date(K.dayStart(S.hwY || np.y, S.hwM || np.m, S.hwD || np.d, TZ).getTime() - 7 * 86400000); const p = K.tzParts(t, TZ); S.hwY = p.y; S.hwM = p.m; S.hwD = p.d; renderNatal(); },
    hwNext() { const t = new Date(K.dayStart(S.hwY || np.y, S.hwM || np.m, S.hwD || np.d, TZ).getTime() + 7 * 86400000); const p = K.tzParts(t, TZ); S.hwY = p.y; S.hwM = p.m; S.hwD = p.d; renderNatal(); },
    hyPrev() { S.hyY = (S.hyY || np.y) - 1; renderNatal(); },
  ...
  function todayHeroHTML() {
    ...
    return `<button type="button" class="hero-today ${da.color}" data-act="jumpDay" data-y="${np.y}" data-m="${np.m}" data-d="${np.d}" aria-label="Otevřít dnešek">
      ...
      ${S.wxOpen ? `<span class="wxpanel top" data-act="noop">${inlineTags(wxPanelHTML())}</span><span class="ht-div"></span>` : ''}
...
<footer>Sestaveno ${new Date().toLocaleDateString('cs-CZ')} · Oáza Adamanthea · oaza-adamanthea.cz<br>Mapa je zrcadlo — popis nástrojů, se kterými jsi přišel.</footer>
  ...
  function horoscopeYear(n) {
    ...
    for (let i = 0; i <= NPTS; i++) {
      ...
      samples.push({ d, lon: Object.fromEntries(TRP.map(b => [b, K.lonOf(b, t)])) });
    }
  ...
  function renderEvents() {
    ...
    setTimeout(() => {
      ...
      const partialKey = key + '|3';
      if (!evs && !S.evAll && !S.evCache[partialKey]) {
        const d0 = K.dayStart(np.y, np.m, 1, TZ); const [ey, em] = monthsList[3]; const d1 = K.dayStart(ey, em, 1, TZ);
        let part = []; for (const [yy, mm] of monthsList.slice(0, 3)) part = part.concat(monthEvents(yy, mm));
        part = part.concat(autoCometEvents(d0, d1)).concat(portalEvents(d0, d1)).sort((a, b) => a.date - b.date);
        S.evCache[partialKey] = part;
        // zbytek roku dopočítat v klidu po měsících
        let i = 3; const step = () => { if (i >= monthsList.length) { delete S.evCache[partialKey]; return; } const [yy, mm] = monthsList[i++]; try { monthEvents(yy, mm); } catch (e) { } setTimeout(step, 60); }; setTimeout(step, 400);
      }
      if (!evs && !S.evAll && S.evCache[partialKey]) evs = S.evCache[partialKey];
      if (!evs) {
        ...
        evs = []; for (const [yy, mm] of monthsList) evs = evs.concat(monthEvents(yy, mm));
        evs = evs.concat(autoCometEvents(d0, d1)).concat(portalEvents(d0, d1));
      ...
      if (!S.evAll && (allKeys.length > 3 || S.evCache[partialKey])) html += `<div class="row" style="justify-content:center;margin:12px 0 4px"><button type="button" class="btn ghost" data-act="evAll">Zobrazit celý rok</button></div>`;
  ...
  function renderNatal() {
    ...
    if (view === 'horoskop') {
      ...
      if (hv === 'tyden') {
        const dy = S.hwY || np.y, dm = S.hwM || np.m, ddd = S.hwD || np.d;
        v.innerHTML = hback('Týdenní') + `<div class="monthbar"><button class="navbtn" data-act="hwPrev" aria-label="Předchozí týden">‹</button><div class="mn">týden<em>${ddd}. ${dm}. ${dy}</em></div><button class="navbtn" data-act="hwNext" aria-label="Další týden">›</button></div>` + weekReadingHTML(dy, dm, ddd); return;
      }
      if (hv === 'mesic') {
        const my = S.mrY || np.y, mm = S.mrM || np.m;
        v.innerHTML = hback('Měsíční') + `<div class="monthbar"><button class="navbtn" data-act="mrPrev" aria-label="Předchozí měsíc">‹</button><div class="mn">${K.MONTH_CZ[mm - 1].charAt(0).toUpperCase() + K.MONTH_CZ[mm - 1].slice(1)}<em>${my}</em></div><button class="navbtn" data-act="mrNext" aria-label="Další měsíc">›</button></div>` + monthReadingHTML(my, mm); return;
      }
      if (hv === 'rok') {
        const yy = S.hyY || np.y;
        v.innerHTML = hback('Roční') + `<div class="monthbar"><button class="navbtn" data-act="hyPrev" aria-label="Předchozí rok">‹</button><div class="mn">${yy}</div><button class="navbtn" data-act="hyNext" aria-label="Další rok">›</button></div>` + yearReadingHTML(yy); return;
      }
    }
    if (view === 'vztahy') {
  ...
  let _warmTimer = null;
  function warmNextMonth(y, m) {
    clearTimeout(_warmTimer);
    let yy = y, mm = m + 1; if (mm > 12) { mm = 1; yy++; }
    const n = K.daysInMonth(yy, mm); let d = 1;
    const step = () => { if (S.y !== y || S.m !== m) return; const t0 = Date.now(); while (d <= n && Date.now() - t0 < 12) { try { analyze(yy, mm, d); } catch (e) { } d++; } if (d <= n) _warmTimer = setTimeout(step, 40); };
    _warmTimer = setTimeout(step, 600);
  }
  function renderCalendar() { renderCalendarRaw(); setTimeout(() => jMediaHydrate(), 30); warmNextMonth(S.y, S.m); }
